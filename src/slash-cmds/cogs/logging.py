...
class logging(commands.Cog):
    ...
    async def ticketlog(self, interaction: discord.Interaction,  modus: typing.Literal["An","Aus"], kanal: discord.TextChannel):
        ...
        async with self.bot.pool.acquire() as conn:
            async with conn.cursor() as cursor:
                if modus == "An":
                    await cursor.execute(f"SELECT channelid FROM ticketlog WHERE guildid = {interaction.guild.id}")
                    result = await cursor.fetchone()
                    if result is None:
                        ...
                        return
                    if result != None:
                        await cursor.execute(f"UPDATE ticketlog SET channelid = {kanal.id} WHERE guildid = {interaction.guild.id}")
                ...
                if modus == "Aus":
                    ...
                    if result is None:
                        ...
                        return
                    if result != None:
                        await cursor.execute(f"DELETE FROM ticketlog WHERE guildid = {interaction.guild.id}")
