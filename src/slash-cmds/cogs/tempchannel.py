...
class owner(discord.ui.Modal, title="Kanalowner vergeben"):
    def __init__(self):
        super().__init__(custom_id="cwhbvohbohucb")
        self.add_item(discord.ui.TextInput(label="Neuer Owner", style=discord.TextStyle.short, required=True, placeholder="eine User ID"))

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.user.voice.channel.edit(name=interaction.user.voice.channel.name, user_limit=self.children[0].value)
        await interaction.response.defer(thinking=False, ephemeral=True)

        user = interaction.guild.get_user(int())
...
class interface(discord.ui.View):
    ...
    @discord.ui.button(style=discord.ButtonStyle.grey, custom_id="ewjhfboqubrouvwfouz", emoji="<:v_krone:1037065917684789279>")
    async def give_owner(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.voice == None:
            return await interaction.response.send_message("**❌ Du bist in keinem Tempchannel.**", ephemeral=True)
        ...
        await interaction.response.send_modal(owner())
