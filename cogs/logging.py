...
class logging(commands.Cog):
    ...
    async def reportlog(self, interaction: discord.Interaction, argument: typing.Literal["Einrichten (Kanal muss mit angegeben werden)","Anzeigen","Ausschalten"], kanal: discord.TextChannel=None):
                ...
                if argument == "Ausschalten":
                    ...
                    if result == None:
                        return await interaction.followup.send("**<:v_9:1264264656831119462> Auf diesem Server ist kein Reportlog eingerichtet.**", ephemeral=True)
                    await db["reportlog"].delete_one({"guildID": interaction.guild.id})
                    return await interaction.followup.send("**<:v_158:1264268251916009553> Der Reportlog wurde ausgeschaltet.**")
                if argument == "Einrichten (Kanal muss mit angegeben werden)":
                    ...
                    await interaction.followup.send(f"**<:v_158:1264268251916009553> Der Reportlog ist nun aktiv in {kanal.mention}.**")
                if argument == "Anzeigen":
                    result = db["reportlog"].find_one({"guildID": interaction.guild.id})
                    try:
                        channel = await interaction.guild.fetch_channel(int(result["channelID"]))
                    except:
                        return await interaction.followup.send("**<:v_9:1264264656831119462> Der Kanal des Reportlogs existiert nicht mehr. Bitte deaktiviere den Reportlog und richte ihn erneut ein.**", ephemeral=True)
        ...
        try:
            ...
            if result != None:
                ...
                if channel is None:
                    ...
                    embed.add_field(name="<:v_56:1264265471339925575> Autor", value=message.author)
        ...
        if result != None:
            ...
            if channel is None:
                ...
                async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                    ...
                    embed.add_field(name="<:v_56:1264265471339925575> User", value=f"{entry.target.mention}({entry.target})")
            ...
            if result != None:
                ...
                if chan is None:
                    ...
                    async for entry in channel.guild.audit_logs(limit=1, action=discord.AuditLogAction.channel_create):
                        ...
                        embed.add_field(name="<:v_56:1264265471339925575> User", value=f"{entry.user.mention}({entry.user})")
                    ...
                    if result != None:
                        ...
                        if len(before.roles) > len(after.roles):
                            ...
                            async for entry in after.guild.audit_logs(action=discord.AuditLogAction.member_role_update, limit=1):
                                ...
                                fields = [("<:v_56:1264265471339925575> Betroffenes Mitglied", before.mention, True),
    ...
    async def ticketlog(self, interaction: discord.Interaction,  modus: typing.Literal["An","Aus"], kanal: discord.TextChannel):
        ...
        if premium == False:
            return await interaction.followup.send("**<:v_9:1264264656831119462> Du kannst dies nicht tun, da der Serverowner kein Premium besitzt. [Premium auschecken](https://vulpo-bot.de/premium)**")
