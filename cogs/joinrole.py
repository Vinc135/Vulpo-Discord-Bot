...
class joinrole(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        if member.bot:
            try:
                result = await db['botroles'].find({"guild_id": str(member.guild.id)}).to_list(length=None)
                if result == None:
                    return
                for role in result:
                    r = discord.utils.get(member.guild.roles, id=int(role['role_id']))
                    if r is not None:
            ...
            try:
                result = await db['joinroles'].find({"guild_id": str(member.guild.id)}).to_list(length=None)
                if result == None:
                    return
                for role in result:
                    r = discord.utils.get(member.guild.roles, id=int(role['role_id']))
    ...
    async def joinrole(self, interaction: discord.Interaction, argument: typing.Literal["Hinzufügen (Rolle muss mit angegeben werden)", "Löschen","Anzeigen"], rolle: discord.Role=None):
        ...
        if argument == "Löschen":
            await db['joinroles'].delete_many({"guild_id": str(interaction.guild.id)})
            await interaction.followup.send("**<:v_checkmark:1264271011818242159> Alle Joinrollen gelöscht.**")           
            return
        if argument == "Hinzufügen (Rolle muss mit angegeben werden)":
            if rolle is None:
                await interaction.followup.send("**<:v_x:1264270921452224562> Eine Rollen-Angabe ist erforderlich beim Einrichten.**", ephemeral=True)           
                return
            ...
            if result:
                await interaction.followup.send("**<:v_x:1264270921452224562> Diese Rolle ist bereits eingestellt.**", ephemeral=True)           
                return
            
            db["joinroles"].insert_one({"role_id": rolle.id, "guild_id": str(interaction.guild.id)})
        ...
        if argument == "Anzeigen":
            ...
            result = await db['joinroles'].find({"guild_id": str(interaction.guild.id)}).to_list(length=None)
            ...
            for r in result:
                r = discord.utils.get(interaction.guild.roles, id=int(r['role_id']))
                if r is not None:
                    rollen += f"{r.mention}\n"
            embed = discord.Embed(colour=await getcolour(self, interaction.user),
    ...
    async def botrole(self, interaction: discord.Interaction, argument: typing.Literal["Hinzufügen (Rolle muss mit angegeben werden)", "Löschen","Anzeigen"], rolle: discord.Role=None):
        ...
        if argument == "Löschen":
            db["botroles"].delete_many({"guild_id": str(interaction.guild.id)})
            await interaction.followup.send("**<:v_checkmark:1264271011818242159> Alle Joinrollen gelöscht.**")           
            return
        if argument == "Hinzufügen (Rolle muss mit angegeben werden)":
            if rolle is None:
            ...
            await db['botroles'].insert_one({"role_id": rolle.id, "guild_id": str(interaction.guild.id)})
            embed = discord.Embed(colour=await getcolour(self, interaction.user),
        ...
        if argument == "Anzeigen":
            ...
            result = await db['botroles'].find({"guild_id": str(interaction.guild.id)}).to_list(length=None)
