...
class buttons(discord.ui.View):
    ...
    async def skip(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(color=await getcolour(interaction.user), title="Flaggenquiz", description="Solltest du Probleme beim Lösen haben, kannst du die Buttons dieser Nachricht benutzen.")
        ...
        await fqcurrent.update_one(
            ...
            {"$set": {"lösung": db_entry["lösung"], "msgID": m2.id}},
            upsert=True
        )
    ...
    async def letter(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        result = await fqcurrent.find_one({"guildID": interaction.guild.id})
        if result:
...
async def check_channel(self, msg):
    ...
    result = await fq.find_one({"guildID": msg.guild.id})
    if result:
...
async def check_word(self, msg: discord.Message):
    ...
    result = await fqcurrent.find_one({"guildID": msg.guild.id})
    if result and result["lösung"].lower() == msg.content.lower():
        await fqcurrent.delete_one({"guildID": msg.guild.id})
        return True
    return False
...
async def answer_correct(self, msg):
    try:
        ...
        await update_account(msg.author, "rucksack", 10, 0)
        ...
        embed = discord.Embed(color=await getcolour(msg.author), title="Flaggenquiz", description="Solltest du Probleme beim Lösen haben, kannst du die Buttons dieser Nachricht benutzen.")
        embed.set_image(url="attachment://Flagge.png")
        ...
        await fqcurrent.insert_one({"guildID": msg.guild.id, "lösung": db_entry["lösung"], "msgID": m2.id})
...
class flaggenquiz(commands.Cog):
    ...
    async def flaggenquiz(self, interaction: discord.Interaction, modus: typing.Literal["Anschalten", "Ausschalten"], kanal: typing.Union[discord.TextChannel, discord.ForumChannel, discord.Thread]):
        ...
        if modus == "Anschalten":
            ...
            if not flag:
                await fq.insert_one({"guildID": server.id, "channelID": kanal.id})
            else:
                await fq.update_one({"guildID": server.id}, {"$set": {"channelID": kanal.id}})
