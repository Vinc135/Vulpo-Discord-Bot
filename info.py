    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async with self.bot.pool.acquire() as conn:
            async with conn.cursor() as cursor:
                ...
                await cursor.execute("INSERT INTO erinnerungen(userID, endtime, zeit, beschreibung, id) VALUES(%s, %s, %s, %s, %s)", (interaction.user.id, t1, self.zeit, self.beschreibung.value, id))
...
async def reminder_end(when: datetime.datetime, bot, user_id, id):
    ...
    async with bot.pool.acquire() as conn:
        async with conn.cursor() as cursor:
            ...
            if user:
                await cursor.execute("SELECT beschreibung, zeit FROM erinnerungen WHERE userID = (%s) AND id = (%s)", (user_id, id))
                result = await cursor.fetchone()
    ...
    async with bot.pool.acquire() as conn:
        async with conn.cursor() as cursor:
            ...
            if status == "Beenden":
                ...
                result = await cursor.fetchone()
            if status == "Reroll":
                await cursor.execute("SELECT * FROM gewinnspiele WHERE msgID = (%s) AND status = (%s)", (msgID, "Inaktiv"))
                result = await cursor.fetchone()
            if result is None:
                return
            #
